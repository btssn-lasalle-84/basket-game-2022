/**
* @file src/main.cpp
* @brief Programme principal B4SKET-GAME 2022
...
const String nomsTrame[TypeTrame::NB_TRAMES] = { "STT", "PAU", "PLY", "STP", "RST", "CFG", "ACK" }; //!< nom des trames dans le protocole
...
CouleurEquipe joueurCourant = CouleurEquipe::ROUGE; //!< la couleur de l'équipe qui tire
const String codeEquipe[CouleurEquipe::NbEquipes] = { "R", "J" }; //!< code de l'équipe qui tire
//String typePartie;
...
void envoyerTrameAcquittement(bool panier=false)
{
  ...
  {
    sprintf((char *)trameEnvoi, "%s;%s;%d;\r", entete.c_str(), nomsTrame[TypeTrame::ACK].c_str(), nbPaniers);
  }
  else //Format : $basket;ACK;\r
  {
    sprintf((char *)trameEnvoi, "%s;%s;\r", entete.c_str(), nomsTrame[TypeTrame::ACK].c_str());
  }
...
void envoyerTrameDetection(int numeroPanier, CouleurEquipe couleurEquipe)
{
  ...
  sprintf((char *)trameEnvoi, "%s;P;%d;%s;\r", entete.c_str(), numeroPanier, codeEquipe[(int)couleurEquipe].c_str());
  ESPBluetooth.write((uint8_t*)trameEnvoi, strlen((char *)trameEnvoi));
  #ifdef DEBUG
  String trame = String(trameEnvoi);
  trame.remove(trame.indexOf("\r"), 1);
...
void setup()
{
  ...
  titre = nomBluetooth;
  #else
  String nomBluetooth = "b4sket-game 2022";
    ...
    switch (typeTrame)
    {
      ...
      case TypeTrame::PAU:
        if(etatPartie == EnCours)
        {
          ...
          digitalWrite(GPIO_LED_VERTE, LOW);
          afficheur.setMessageLigne(Afficheur::Ligne3, String("En pause"));
          afficheur.afficher();
        }
        break;
      case TypeTrame::PLY:
